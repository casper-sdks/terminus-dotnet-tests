...
public class StateGetBalanceStepDefinitions {
    ...
    private Nctl _nctl = new(TestProperties.DockerName);
    ...
    public async Task GivenThatTheStateGetBalanceRpcMethodIsInvokedAgainstNcltUserPurse() {
        ...
        var stateRootHash =  _nctl.GetStateRootHash(1);
    ...
    public async Task ThenTheStateGetBalanceResultContainsThePurseAmount() {
        ...
        var accountMainPurse = _nctl.GetAccountMainPurse(1);
        var json = await _simpleRcpClient.GetBalance(_nctl.GetStateRootHash(1), accountMainPurse);
